...
class DemoPage {
    ...
    observeInput(element: HTMLElement, range: boolean, min: number, max: number,
      ...
      const changeSlider = (evt: MouseEvent): void => {
        ...
        const settingValue: boolean | number | undefined | HTMLElement = this.validateSettings(
    ...
    number | undefined {
      ...
      if (valueToNumber && (valueToNumber >= min && valueToNumber <= max)) {
    ...
    validateSettings(value: string, element: HTMLElement):
     boolean | number | undefined | HTMLElement {
      const convertedValue: boolean | number | undefined = DemoPage.convertInputValue(value);
      if (convertedValue && typeof convertedValue === 'number') {
        return convertedValue;
      }
      switch (true) {
        case element.classList.contains('form__input-settings--progress')
      ...
      || element.classList.contains('form__input-settings--range'):
        ...
        case element.classList.contains('form__input-settings--min')
      || element.classList.contains('form__input-settings--max'):
        ...
        case element.classList.contains('form__input-settings--step'):
          DemoPage.deleteErrorElement(element);
          return (typeof convertedValue === 'undefined' || typeof convertedValue === 'number') ? convertedValue
    ...
    createErrorElement(element: HTMLElement, text: string): HTMLElement {
      this.errorElement = document.createElement('span');
      this.errorElement.textContent = text;
      this.errorElement.classList.add('error');
      return ((element as HTMLElement).parentElement as HTMLElement)
